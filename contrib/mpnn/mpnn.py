...
# DATASET = 'az_ppb.csv'
DATASET = "/home/derek/workspace/deepchem/datasets/delaney-processed.csv"
print(DATASET)
...
def readout(h, h2):
    ...
    activated_reads = map(lambda x: F.relu(R(x)), catted_reads)
...
for i in range(0, MAXITER):
    ...
    if i % int(len(train_smiles) / BATCH_SIZE) == 0:
        ...
        y_hats_val = torch.stack(y_hats_val).view(-1).data.numpy()
        y_val = val_labels.data.numpy()
